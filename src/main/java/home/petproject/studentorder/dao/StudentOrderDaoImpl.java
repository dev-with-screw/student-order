...
public class StudentOrderDaoImpl implements StudentOrderDao{
    ...
    private static final String INSERT_CHILD = 
        "INSERT INTO jc_student_child(" +
                ...
                " c_patronymic, c_date_of_birth, c_sertificate_number, c_sertificate_date, " +
    ...
    public Long saveStudentOrder(StudentOrder so) throws DaoException {
        ...
        try (Connection con = getConnection();
            ...
            try {
                ...
                //Marriage
                stmt.setString(33, so.getMarriageCertificatedId()); // certificate_id varchar(20) not null,
                stmt.setInt(34, so.getMarriageOffice().getOfficeID().intValue());// register_office_id integer not null,
                stmt.setDate(35, java.sql.Date.valueOf(so.getMarriageDate()));// marriage_date date not null,
...
//        return getStudentOrdersTwoSelect();
    ...
    private void fillStudentOrder(ResultSet rs, StudentOrder so) throws SQLException {
        so.setStudentOrderId(rs.getLong("student_order_id"));//student_order_id serial,
        so.setStudentOrderStatus(StudentOrderStatus.fromValue(rs.getInt("student_order_status")));//student_order_status int not null,
        so.setStudentOrderDate(rs.getTimestamp("student_order_date").toLocalDateTime());//student_order_date timestamp not null,
    }

    private Adult fillAdult(ResultSet rs, String pref) throws SQLException {
        Adult adult = new Adult();

        adult.setSurName(rs.getString(pref + "sur_name"));//h_sur_name varchar(100) not null,
        adult.setGivenName(rs.getString(pref + "given_name"));//h_given_name varchar(100) not null,
        adult.setPatronymic(rs.getString(pref + "patronymic"));//h_patronymic varchar(100) not null,
        adult.setDateOfBirth(rs.getDate(pref + "date_of_birth").toLocalDate());//h_date_of_birth date not null,
        adult.setPassportSeria(rs.getString(pref + "passport_seria"));//h_passport_seria varchar(10) not null,
        adult.setPassportNumber(rs.getString(pref + "passport_number"));//h_passport_number varchar(10) not null,
        adult.setIssueDate(rs.getDate(pref + "passport_date").toLocalDate());//h_passport_date date not null,
        ...
        adult.setIssueDepartment(po);//h_passport_office_id integer not null,

        Address adr = new Address();
        Street st = new Street(rs.getLong(pref + "street_code"), "");//h_street_code integer not null,
        adr.setStreet(st);
        adr.setPostCode(rs.getString(pref + "post_index"));//h_post_index varchar(10),
        adr.setBuilding(rs.getString(pref + "building"));//h_building varchar(10) not null,
        adr.setExtension(rs.getString(pref + "extension"));//h_extension varchar(10),
        adr.setApartment(rs.getString(pref + "apartment"));//h_apartment varchar(10),
        adult.setAddress(adr);

        University uni = new University(rs.getLong(pref + "university_id"), "");
        adult.setUniversity(uni); //h_university_id integer not null,

        adult.setStudentID(rs.getString(pref + "student_number"));//h_student_number varchar(30) not null,

        return adult;
    }

    private void fillMarriage(ResultSet rs, StudentOrder so) throws SQLException {
        so.setMarriageCertificatedId(rs.getString("certificate_id"));//certificate_id varchar(20) not null,
        so.setMarriageDate(rs.getDate("marriage_date").toLocalDate());//marriage_date date not null,
        Long roId = rs.getLong("register_office_id");//register_office_id integer not null,
    ...
    private Child fillChild(ResultSet rs) throws SQLException {
        ...
        child.setCertificateNumber(rs.getString("c_sertificate_number"));
        child.setIssueDate(rs.getDate("c_sertificate_date").toLocalDate());
